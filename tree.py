...
class Tree:
    ...
    def __str__(self)->str:
        ...
        DotExporter(self.rootNode,
                    nodenamefunc=lambda node: node.node_value,
                    nodeattrfunc=lambda node: "style = filled, fillcolor = red" if node.depth % 2 == 0 else "style = filled, fillcolor = blue",
                    edgeattrfunc=lambda node, child: "label =" + str(node.evaluator_value)
                    ).to_dotfile("test.dot")
